...
simu.multi3 <- function(Gnull, Gsig, alpha, Ntrain, Ntest, SDgenemu=5, DFgenesd=1, Clow=2, Cup=Inf, MuShift=0, SDshift=1, pShift=0.3,seed=15213){
  ### input
  ...
  if(alpha<0 | alpha>1){
    stop("Ratio alpha has to be [0,1]")
  }
  ...
  if(length(Ntrain)!=3){
    stop("Training data have to have 3 outcomes, that is length(Ntrain)=3")
  }
  ...
  ##### sig gene
  ## only-one-DE gene, alpha type, type 1
  Galpha=as.integer(Gsig*alpha)
  if(Galpha>0){
    shiftC1=matrix(rtruncnorm(Galpha*K,a=Clow,b=Cup,mean=0,sd=SDgenemu),nrow=Galpha,ncol=K)
    directionCandidate1=rbind(c(1,0,0),c(0,1,0),c(0,0,1),c(-1,0,0),c(0,-1,0),c(0,0,-1))
    direction1=matrix(0,nrow=Galpha,ncol=K)  # showing the gene direction for each outcome category
    for(i in 1:Galpha){
      direction1[i,]=directionCandidate1[sample(nrow(directionCandidate1),1),] # randomly choose from candidate
      
      for(j in 1:K){
        E[i,NstartInd[j]:NendInd[j]]=E[i,NstartInd[j]:NendInd[j]] + direction1[i,j]*shiftC1[i,j]
      }
    }
  }else{
    direction1=c()
  ...
  ## all-DE gene, 1-alpha type, type 2
  GalphaN=Gsig-Galpha
  if(GalphaN>0){
    shiftC2=matrix(rtruncnorm(GalphaN*K,a=Clow,b=Cup,mean=0,sd=SDgenemu),nrow=GalphaN,ncol=K)
    directionCandidate2=rbind(c(1,0,-1),c(1,-1,0),c(0,-1,1),c(0,1,-1),c(-1,1,0),c(-1,0,1))
    direction2=matrix(0,nrow=GalphaN,ncol=K)  # showing the gene direction for each outcome category
    for(i in 1:GalphaN){
      direction2[i,]=directionCandidate2[sample(nrow(directionCandidate2),1),] # randomly choose from candidate
      
      for(j in 1:K){
        E[i+Galpha,NstartInd[j]:NendInd[j]]=E[i+Galpha,NstartInd[j]:NendInd[j]] + direction2[i,j]*shiftC2[i,j]
      }
    }
  }else{
    direction2=c()
  ...
  # sig gene index
  sigIndex=rep(0,times=Gtotal)
  sigIndex[1:Gsig]=rep(c(1,2),times=c(Galpha,GalphaN))
  ...
  if(Gsig>0){
    geneMUtrain[1:Gsig,]=geneMUtrain[1:Gsig,]+direction*shiftC
  }  
...
out=simu.multi3(Gnull, Gsig, alpha, Ntrain, Ntest, pShift=0.5, SDshift=2)
