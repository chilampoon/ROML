---
title: "LumA vs LumB - Random Forest"
author: "Poon Chi Lam"
date: "6/18/2019"
...
# Get all results
doRF <- function(ori.train, ori.test, lb.train, lb.test, featureSet) {
  results <- list()
  s.train <- ori.train[featureSet,]
  s.test <- ori.test[featureSet,]
  
  results[['cv']] <- rf.CV(as.data.frame(t(s.train)), lb.train)
...
### 1. All DEGs
tcga.all <- doRF(nm.data.train, nm.data.test, lb.train, lb.test, rownames(nm.data.train))

### 2. Super DEGs
sDEG <- rownames(tt[(tt$adj.P.Val < 0.01) & (abs(tt$logFC) > log2(1.5)), ]) 
tcga.super <- doRF(nm.data.train, nm.data.test, lb.train, lb.test, sDEG)

### The rest of the feature sets
for (s in c(50, 100, 500, 1000, 2000)) {
  print(paste0('Top ', s, ' DEGs...'))
  set <- rownames(tt[1:s,])
  assign(paste0('tcga.', s), doRF(nm.data.train, nm.data.test, lb.train, lb.test, set))
}

save(tcga.all, tcga.super, tcga.50, tcga.100, tcga.500, tcga.1000, tcga.2000, file=file.path(data.dir, 'tcga.RF.results.Rdata'))
```
...
### 1. All DEGs
mb.all <- doRF(nm.data.train, nm.data.test, lb.train, lb.test, rownames(nm.data.train))

### 2. Super DEGs
sDEG <- rownames(tt[(tt$adj.P.Val < 0.01) & (abs(tt$logFC) > log2(1.5)), ]) 
mb.super <- doRF(nm.data.train, nm.data.test, lb.train, lb.test, sDEG)

### The rest of the feature sets
for (s in c(50, 100, 500, 1000, 2000)) {
  print(paste0('Top ', s, ' DEGs...'))
  set <- rownames(tt[1:s,])
  assign(paste0('mb.', s), doRF(nm.data.train, nm.data.test, lb.train, lb.test, set))
}

save(mb.all, mb.super, mb.50, mb.100, mb.500, mb.1000, mb.2000, file=file.path(data.dir, 'mb.RF.results.Rdata'))
```


## Visulization
```{R vis, fig.width=12, fig.height=4}
#load(file.path(data.dir,'mb.RF.results.Rdata'))
#load(file.path(data.dir,'tcga.RF.results.Rdata'))

draw.perf <- function(name) {
  plot.df <- data.frame(fs=c('all', 'super', '50', '100', '500', '1000', '2000'), 
                      value=rep(0, 7))
  # CV acc
  plot.df$value <- sapply(plot.df$fs, function(x) get(paste0(name,'.', x))[['cv.overall']][['ACC']])
  g1 <- ggplot(plot.df, aes(x=fs, y=value, group=1)) + geom_line(size=1.1, color='tomato') + 
      geom_point(size=1.1) + theme_bw() + labs(y='CV ACCs') + ggtitle(paste0(name, ' CV ACC')) +
      scale_x_discrete(limits = plot.df$fs) +
      scale_y_continuous(breaks = seq(0,1, by=0.1), limits = c(0,1))
  
  plot.df <- data.frame(fs=c('all', 'super', '50', '100', '500', '1000', '2000'), 
                        value=rep(0, 7))
  # Testing acc
  plot.df$value <- sapply(plot.df$fs, function(x) get(paste0(name,'.', x))[['test']][['ACC']])
  g2 <- ggplot(plot.df, aes(x=fs, y=value, group=1)) + geom_line(size=1.1, color='skyblue') + 
      geom_point(size=1.1) + theme_bw() + labs(y='Testing ACCs') +
      scale_x_discrete(limits = plot.df$fs) + ggtitle(paste0(name, ' testing ACC')) +
      scale_y_continuous(breaks = seq(0,1, by=0.1), limits = c(0,1))
