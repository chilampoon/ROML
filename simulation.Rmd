...
```{R}
# Calculate tsp scores in one vs all other way
getTSP.1 <- function(grps, expr.train, lb.train) {
  tsp.list <- list()
  for (grp in grps) {
    try({
      grp.vec <- rep(0, length(lb.train))
      grp.vec <- factor(replace(grp.vec, which(lb.train == grp), 1)) # one v.s. all other
      tmp <- SWAP.CalculateScores(as.matrix(expr.train), grp.vec, FilterFunc = NULL)
...
getTSP.2 <- function(grps, expr.train, lb.train) {
  tsp.list <- list()
  for (i in 1:length(grps)) {
    if (i != length(grps)) {
      for (j in (i+1):length(grps)) {
        exp1 <- expr.train[ ,lb.train == as.character(grps[i])]
        exp2 <- expr.train[ ,lb.train == as.character(grps[j])]
        grp.data <- cbind.data.frame(exp1, exp2)
        grp.vec <- factor(lb.train[lb.train %in% c(as.character(grps[i]), as.character(grps[j]))])
...
mergePairs <- function(Ks, tsp.list) {
  kpairs <- list()
...
trainModel <- function(dataframe, labels, Ks, kpairs) {
  model <- list()
  for (K in Ks) {
...
g.null <- 80
g.sig <- 20
n.train <- c(50, 70, 85) # class 0,1,2
n.test <- c(60, 75, 80)
SDnullMu <- 5
DFnullSD <- 1
lb.shift <- 0.8
...
#### Using one vs all other tsp scores
```{R ML1}
for (i in 1:length(out.sets)) {
  alpha <- names(out.sets)[i]
  print(paste0('The percentage of only-one-DE alpha = ', alpha))
  out <- out.sets[[i]]
...
for (i in 1:length(out.sets)) {
  ...
  accp <- plot.perf(overall.cv2, Ks, c(0, 1), 'ACC', 'ACC')
  ydp <- plot.perf(overall.cv2, Ks, c(0, 1), 'Avg. Youden Index', 'avg.yd')
  sensp <- plot.perf(overall.cv2, Ks, c(0, 1), 'Avg. Sensitivity', 'avg.sens')
  specp <- plot.perf(overall.cv2, Ks, c(0, 1), 'Avg. Specificity', 'avg.spec')
  fig <- ggarrange(accp, ydp, sensp, specp, nrow=2, ncol=2)
  print(annotate_figure(fig, top='5 fold cross-validation'))
  ...
  accp <- plot.perf(test.results2, Ks, c(0, 1), 'ACC', 'ACC')
  ydp <- plot.perf(test.results2, Ks, c(0, 1), 'Avg. Youden Index', 'avg.yd')
  sensp <- plot.perf(test.results2, Ks, c(0, 1), 'Avg. Sensitivity', 'avg.sens')
  specp <- plot.perf(test.results2, Ks, c(0, 1), 'Avg. Specificity', 'avg.spec')
  fig <- ggarrange(accp, ydp, sensp, specp, nrow=2, ncol=2)
  print(annotate_figure(fig, top='Testing'))
}
```
