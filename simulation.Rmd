...
tsp.rf.cv <- function(nfold, Ks, labels, dataframe, grp.names, kpairs, no.model) {
  ...
  results.list <- list()
  grp.names <- as.character(grp.names)
...
for (a in alpha) {
  ...
  for (i in 1:2) {
    ...
    all.results[[as.character(a)]][[chri]][['test']] <- testing(as.data.frame(t(expr.test)), lb.test, 
                                          Ks, grps, kpairs[[i]], all.results[[as.character(a)]][[chri]][['full.model']], i)
  }
...
plot.perf <- function(modelName, a.results, Ks, name, listname, whichone) {
  ...
  for (n in modelName) {
    ...
    for (k in 1:length(Ks)) {
      ...
      df[nrow(df), ]$model <- paste0('model ', n)
      df[nrow(df), ]$K <- K
    }
  }
  rownames(df) <- NULL
