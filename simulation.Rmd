...
g.null <- 900
g.sig <- 100
n.train <- c(200, 200, 200) # class 0,1,2
n.test <- c(200, 200, 200)
SDnullMu <- 5
DFnullSD <- 4
lb.shift <- 0.7
ub.shift <- 1.2
...
### Baseline model
Input raw expression values into random forest model.
```{R baseline}
# 5-fold cross validation
rf.cv <- function(data.train, labels.train) {
  results.list <- list()
...
bl.model <- function(data.train, labels.train) {
  data.train <- cbind.data.frame(data.train, type=labels.train)
...
raw.testing <- function(data.test, labels.test, model) {
  test.results <- list()
...
for (a in alpha) {
  ...
  all.results[[as.character(a)]][['baseline']][['cv']] <- rf.cv(as.data.frame(t(expr.train)), lb.train)
  all.results[[as.character(a)]][['baseline']][['overall.cv']] <- getOverall(all.results[[as.character(a)]][['baseline']][['cv']], Ks, baseline=TRUE)
  all.results[[as.character(a)]][['baseline']][['full.model']] <- bl.model(as.data.frame(t(expr.train)), lb.train)
  all.results[[as.character(a)]][['baseline']][['test']] <- raw.testing(as.data.frame(t(expr.test)), lb.test, 
                                                    all.results[[as.character(a)]][['baseline']][['full.model']])
  ...
  for (i in 1:3) {
    ...
    all.results[[as.character(a)]][[chri]][['cv']] <- tsp.rf.cv(5, Ks, lb.train, as.data.frame(t(expr.train)), grps, kpairs[[i]], i)
    ...
    all.results[[as.character(a)]][[chri]][['full.model']] <- trainModel(as.data.frame(t(expr.train)), lb.train, Ks, grps, kpairs[[i]], i)
    
    # Test on independent set
    all.results[[as.character(a)]][[chri]][['test']] <- testing(as.data.frame(t(expr.test)), lb.test, Ks, grps, 
                                          kpairs[[i]], all.results[[as.character(a)]][[chri]][['full.model']], i)
  }
